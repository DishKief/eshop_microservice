...
import { handleForgotPassword, sendOtp, trackOtpRequests, validateRegistrationData, verifyOtp } from "../utils/auth.helper";
...
export const verifyUser = async (req:Request, res:Response, next:NextFunction) => {
    try {
        ...
        if(!email || !otp || !password || !name) {
            return next(new ValidationError("All fields are required for verification!"));
        }
...
export const loginUser = async (req:Request, res:Response, next:NextFunction) => {
    try {
        ...
        if(!email || !password) {
            return next(new ValidationError("Email and password are required for login!"));
        }
